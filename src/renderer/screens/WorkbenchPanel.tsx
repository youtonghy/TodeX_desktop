...
export function WorkbenchPanel({ session, tab, target, onTabChange, scopeKey = session.activeConversation?.id || '', onTargetConsumed, closeRequestRef }: Props) {
  ...
  useEffect(() => {
    let cancelled = false;
    void window.todexDesktop.store.get(WORKBENCH_TAB_AXIS_KEY)
      .then((value) => {
  ...
  const toggleAxis = () => {
    ...
    setAxis(next);
    void window.todexDesktop.store.set(WORKBENCH_TAB_AXIS_KEY, next)
      .catch((reason) => {
  ...
  const newTabDropdown = (
    <Dropdown>
      <Dropdown.Trigger
        isDisabled={!restored} aria-label={t('workbench.newTab')}
