...
- **桌面三栏工作台界面**：
  ...
  - **中央聊天面板（Chat Panel）**：完整会话时间线、流式 Markdown 实时渲染、Shiki 语法高亮、KaTeX 数学公式渲染、交互式审批卡片（命令执行、文件变更对比、工具调用），以及支持模型选择、可拖动思考强度、Codex Fast 模式、`@` 文件引用、`/` 斜杠命令与 `#` Skill/MCP 建议的输入框。
