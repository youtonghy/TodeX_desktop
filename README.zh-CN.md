...
- **桌面三栏工作台界面**：
  ...
  - **中央聊天面板（Chat Panel）**：完整会话时间线、流式 Markdown 实时渲染、Shiki 语法高亮、KaTeX 数学公式渲染、交互式审批卡片（命令执行、文件变更对比、工具调用），以及支持模型搜索选择（按名称匹配，不区分大小写）、可拖动思考强度、Codex Fast 模式、`@` 文件引用、`/` 斜杠命令与 `#` Skill/MCP 建议的输入框。推理与工具详情仅在用户展开后按需挂载，最终答复与上下文、执行事件保持独立。
